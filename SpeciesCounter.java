...
public class SpeciesCounter {
    ...
    public void countSpecies() {
        ...
        // Skip index 0 because it's the header row ("species")
        for (i = 1; i < speciesData.length; ++i) {
            species = speciesData[i].trim();

            // In case the CSV has quotes around values
            if (species.startsWith("\"") && species.endsWith("\"") && species.length() >= 2) {
                species = species.substring(1, species.length() - 1).trim();
            }
